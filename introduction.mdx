...
Learn about Ship [architecture](/architecture) and key components([Web](/web/overview), [API](/api/overview), [Deployment](/deployment/overview), [Migrator](/migrator), [Scheduler](/scheduler).

## .NET

There is a .NET version of Ship. Documentation does not cover it, but [Web](/web/overview) and [Deployment](/deployment/overview) sections will be useful because they are the same as for Node.js.
